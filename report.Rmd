...
plotFactorAspectCombination <- function(paramFactorKey, paramAspectKey) {
  ...
  plot <- ggplot(combination, aes(x=rating)) + 
    ...
    #geom_histogram(stat= "count", binwidth=0.1, breaks = seq(-2, 2, by = 1) ) +
...
withTableStyling(kable((impactValidation %>% filter(factorKey %in% factorKeysValidated))[,columnSelection], 
      caption = "Factors with successfully validated initially stated impacts",
...
withTableStyling(kable((impactValidation %>% filter(factorKey %in% factorKeysPotentiallyValidated))[,columnSelection], 
...
# filter by invalid impacts
#impactValidation %>% filter(validation == "✗" | validation == "(✗)") %>% arrange(desc(validation))
...
withTableStyling(kable((impactValidation %>% filter(factorKey %in% factorKeysRemaining))[,columnSelection], 
...
```{r inverseImpacts, include=FALSE,  echo = FALSE, message=FALSE}
withTableStyling(kable((impactValidation %>% filter(validation == "⇄" | validation == "(⇄)"))[,columnSelection], 
      caption = "Initially stated impacts which are (potentially) inverse",
      col.names = columnNames,
      longtable = FALSE,
      align = columnAlignment
      ))
...
interesting <- newImpacts %>% filter(derivedImpact == "0" | derivedImpact == "(0)")
#interesting$factorName <- sapply(interesting$factorKey, function(key) values(factorInformation, keys=key)[1])
#interesting$aspectName <- sapply(interesting$aspectKey, function(key) values(aspectInformation, keys=key)[1])
#interesting$highLevelAspectName <- sapply(interesting$aspectKey, function(key) values(aspectInformation, keys=key)[4])
